...
export function createScatterDensityRenderer(
  ...
  // DataStore content hash; equal-N rewrite with same buffer identity.
  let lastContentVersion = Number.NaN;
  ...
  const prepare: ScatterDensityRenderer['prepare'] = (
    ...
    // Write uniforms (scale affine used both for dirty gate and uniforms).
    const rb = rawBounds;
    ...
    // When omitted, leave the prior version alone (callers that own DataStore
    // should always pass getSeriesContentHash).
    if (contentVersion !== undefined && lastContentVersion !== contentVersion) {
