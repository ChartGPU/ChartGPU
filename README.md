...
The `grouped-bar` example demonstrates multiple bar series side-by-side per x-category (including negative values) and bar layout options (`barWidth`, `barGap`, `barCategoryGap`). See [grouped-bar/main.ts](examples/grouped-bar/main.ts).
