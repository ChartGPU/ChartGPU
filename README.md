...
- 📈 Multiple series types: line, area, bar, scatter, pie, candlestick, **ohlc** bars, heatmap, **band**, **errorBar**, **impulse**, **step/digital** line & mountain (`step` on line/area), **stacked mountain** (`stack` on line/area), plus **3D** `pointCloud3d` / `surface3d`
...
- 📶 Step / digital line + mountain (`step` on `line`/`area`, `true` ≡ `'after'`) — SciChart `isDigitalLine`; stairs not diagonals; see [`examples/step-line/`](examples/step-line/)
- 📍 Impulse / stem (`type: 'impulse'`) — vertical stems baseline→y with optional markers; SciChart `FastImpulseRenderableSeries`; see [`examples/impulse/`](examples/impulse/)
- 🗻 Stacked mountain / area — multi-series composition fill via `stack` on `line`+`areaStyle` or `type: 'area'` (not single-series mountain; not `type: 'band'`); see [`docs/api/options.md`](docs/api/options.md#stacked-mountain--area-stack-on-line-mountain-or-area) and [`examples/stacked-mountain/`](examples/stacked-mountain/)
