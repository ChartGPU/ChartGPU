...
- **Y-only DataStore rewrite (lines)**: When length is stable and every x matches the previous pack, only y floats are rewritten in CPU staging. GPU `writeBuffer` is still a full interleaved upload (WebGPU has no strided partial write).
