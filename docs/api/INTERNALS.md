...
8. **Unsorted full rewrite (group 3)**: `line` + `sampling: 'none'` + non-monotonic x every frame. No LTTB, no y-only, no `indexSortedProven`. Pack uses specialized tuple path; draw may use `resolveLineDrawPolicy` denseThin width. Main/overlay MSAA stay **4×** — dense wins come from draw policy (not reduced MSAA). **Never** use portable `sampleCount: 2` (WebGPU allows only 1 or 4; 2 fails validation).
9. **Dense scatter draw (group 2)**: `resolveScatterDrawPolicy` — draw-only radius compact when density high; upload dual-buffer full xy; harness remains `sampling: 'none'`.
