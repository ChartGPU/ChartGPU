...
- **Stacked mountain / area** (`stack` on `line`+`areaStyle` or `type: 'area'`) — multi-series composition fill where each layer sits on the cumulative sum of layers below (SciChart stacked mountain parity). Same `stack` string semantics as bars; series order = bottom→top; pos/neg independent stacks from 0; auto Y includes stacked totals; tooltip layer contribution + `stackTotal`; hit topmost layer; non-empty `stack` not GPU-decimation eligible. Not single-series mountain (suite group 8), not `type: 'band'`, not a new SeriesType. Example: `examples/stacked-mountain/`. Docs: `docs/api/options.md` Stacked mountain / area.
- **Error bars** (`type: 'errorBar'`) — SciChart-style HLC per-point high/low whiskers around a measured center (not band fill, not OHLC open/close). Absolute `{x,y,high,low}` + relative `{x,y,yError}` / `{yErrorHigh,yErrorLow}`; `errorMode` both/high/low; vertical + horizontal; `capWidth` CSS px or % of category step; `drawWhiskers` / `drawConnector` / optional `showCenter`; sampling `'none'` only; bounds include whiskers; tooltip `high`/`low`/`yError*`; `appendData` HLC/relative with FIFO. Instanced GPU stems + caps (`errorBar.wgsl`). Example: `examples/error-bars/`. Docs: `docs/api/options.md` ErrorBarSeriesConfig.
