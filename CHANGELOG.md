...
### Changed
- **G7 10M FIFO cold seed (setup &lt; 3s hang gate)** — empty chart → first `appendData(..., { maxPoints: N })` with batch length ≥ N is a **one-shot ring seed**: allocate GPU buffer at exact capacity, single pack + `writeBuffer`, ring mode on immediately (no linear multi‑M residency then promote; no dual-pack fallthrough when DataStore has no series yet). Multi‑M strict-replace content version is O(1) (same stamp policy as cold `setSeries`). Suite harness seeds G7 via `appendData` + `maxPoints` (SciChart `fifoCapacity` parity) instead of full-data `setOption`. Production G7 10M×5: setup First Frame ~580 ms (was HANGING ~3300 ms), steady ~74 Avg FPS. Docs: `docs/performance.md`.
...
### Added
- **Step / digital line + mountain** (`step?: boolean | StepMode` on `line` / `area`) — stairs instead of diagonals (`true` ≡ `'after'`; also `'before'` / `'middle'`). Stroke + mountain fill follow the stepped top edge; works with `stack`. Hit-test/tooltip use **source samples** (not densified corners). Not GPU-decimation eligible; forces standard AA when step active (not dense hairline). Example: `examples/step-line/`. Docs: `docs/api/options.md` LineSeriesConfig `step`.
- **Impulse / stem series** (`type: 'impulse'`) — vertical stems from `baseline` → y per sample, optional lollipop markers (`showMarker` default true). XY data only (not errorBar HLC / not OHLC). Stem width CSS px; bounds include baseline; tooltip `baseline`; `appendData` + FIFO. Example: `examples/impulse/`. Docs: `docs/api/options.md` ImpulseSeriesConfig.
- **Stacked mountain / area** (`stack` on `line`+`areaStyle` or `type: 'area'`) — multi-series composition fill where each layer sits on the cumulative sum of layers below. Same `stack` string semantics as bars; series order = bottom→top; pos/neg independent stacks from 0; auto Y includes stacked totals; tooltip layer contribution + `stackTotal`; hit topmost layer; non-empty `stack` not GPU-decimation eligible. Not single-series mountain (suite group 8), not `type: 'band'`, not a new SeriesType. Example: `examples/stacked-mountain/`. Docs: `docs/api/options.md` Stacked mountain / area.
- **Error bars** (`type: 'errorBar'`) — HLC per-point high/low whiskers around a measured center (not band fill, not OHLC open/close). Absolute `{x,y,high,low}` + relative `{x,y,yError}` / `{yErrorHigh,yErrorLow}`; `errorMode` both/high/low; vertical + horizontal; `capWidth` CSS px or % of category step; `drawWhiskers` / `drawConnector` / optional `showCenter`; sampling `'none'` only; bounds include whiskers; tooltip `high`/`low`/`yError*`; `appendData` HLC/relative with FIFO. Instanced GPU stems + caps (`errorBar.wgsl`). Example: `examples/error-bars/`. Docs: `docs/api/options.md` ErrorBarSeriesConfig.
- **Heatmap column-append / spectrogram streaming** (`chart.updateHeatmap`) — `HeatmapUpdate` modes `replaceZ`, `appendColumns` (+`scrollX`), `appendRows` (+`scrollY`). Full field row-major; appendColumns strips column-major (mirrors `updateSurface3D`). Steady-state single-column scroll uses a modular GPU texture ring + O(rows) strip `writeTexture` (not full-grid upload). Stream vs `setOption` identity policy matches surface3d. `appendData` still unsupported for heatmap (warning points at `updateHeatmap`). Example live path: `examples/heatmap-spectrogram/`. Docs: `docs/api/options.md` HeatmapSeriesConfig streaming.
- **OHLC bars** (`type: 'ohlc'`) — thin open–high–low–close bars (center stem + left open / right close ticks) sharing candlestick `OHLCDataPoint` data, `sampling: 'none' | 'ohlc'`, append/FIFO, `priceLabel`, and finance-primary gutters/right Y. Hit-test `kind: 'ohlc'`. Example: `examples/ohlc-bars/` (candlestick ↔ ohlc toggle on the same data). Docs: `docs/api/options.md` OhlcSeriesConfig.
- **3D GPU axis labels** (`axes3d.labelMode: 'gpu' | 'auto'`) — real WebGPU glyph-atlas billboards for tick numbers + titles (canvas-baked Latin/units charset, theme `textColor`, depth-tested after axis lines). `'auto'` prefers GPU when atlas init succeeds; `'dom'` keeps the P6 DOM path; exclusive (never both). Atlas failure falls back to DOM with a one-time `console.warn`. Camera-only orbit updates `viewProj` only (no atlas rebuild). Docs: `docs/api/3d.md`.
- **3D production polish (P6)** — labeled axes (GPU box/grid/ticks + DOM projected titles/numbers), surface ray pick, exact large-N cloud pick via screen-space grid, `click`/`mouseover`/`mouseout` on 3D canvas, **`maxPoints` FIFO** for `pointCloud3d` (pack rewrite via `planMaxPointsWindow`), **`updateSurface3D`** (`replaceY` / `appendColumns`+`scrollX` / `appendRows`+`scrollZ`), surface **contours** (marching squares, depth-tested), hero demo `examples/3d-showcase/`. Docs: `docs/api/3d.md`.
- **3D chart modality** (`coordinateSystem: 'cartesian3d'`) — separate WebGPU path with depth buffer, perspective/orthographic camera, orbit/pan/zoom, AABB fit, and axis box. Does **not** enable depth on 2D charts. Series: **`pointCloud3d`** (XYZ billboards, solid or colormap-by-value, nearest-point tooltip/pick) and **`surface3d`** (uniform XZ height field, Y-up mesh, colormap, lighting, wireframe). OptionResolver rejects mixing 2D/3D series in one chart. API: `resetCamera` / `setCamera` / `getCamera`; docs `docs/api/3d.md`. Examples: `examples/point-cloud-3d/`, `examples/surface-3d/`, `examples/3d-showcase/`. No Three.js/WebGL dependency.
- **Band / range series** (`type: 'band'`) — GPU fill between dual Y curves sharing x (`y` + `y1`) for confidence intervals, min–max envelopes, and threshold fills. Data formats: Xyy arrays, `[x,y,y1]` tuples/objects, or interleaved stride-3. Optional dual edge strokes (`lineStyle` / `lineStyleY1`), CPU sampling (LTTB on midline; min/max envelope), axis bounds from both channels, tooltips with `y1`/`yMid`/`yRange`, and `appendData` with FIFO. Not area-to-baseline and not annotation `bandX`. Example: `examples/band-range/`.
- **Uniform heatmap / spectrogram series** (`type: 'heatmap'`) — data-space grid of z-values rendered as a GPU `r32float` texture + colormap LUT + single quad (not scatter `mode: 'density'`). Supports viridis/plasma/inferno/magma/grayscale and custom stops, linear/log z scale, corner/center cell anchors, NaN null-handling, zoom/pan without z re-upload, equal-size `setOption` z updates, and cell tooltips (`TooltipParams.z`). Streaming via `updateHeatmap` (see Unreleased heatmap column-append). `appendData` is unsupported (warn). Example: `examples/heatmap-spectrogram/`.
...
### Changed
- **surface3d streaming performance** - Steady-state `updateSurface3D({ mode: 'replaceY' })` no longer allocates a full height field or CPU-packs 32 B interleaved vertices every frame. Full `Float32Array` payloads are **zero-copy** retained; GPU uploads **height-only** storage (4 B/cell) and expands position + normals in the vertex shader. Explicit series `yMin`/`yMax` skips full-field domain walks; AABB XZ is reused on replaceY; contour invalidate is a no-op when contours are off. Prefer a retained typed array + `replaceY` for animated surfaces. Docs: `docs/api/3d.md`.
- **Phase B multi‑M FIFO decimation speed (G0–G2 preserved)** - Honest every-frame LTTB at extreme density is cheaper: dense-bucket candidate cap for LTTB/min/max is **128** (was 512) when pts/bucket > 512; averages pre-pass uses **64** coarse anchors; modular ring `rawAt` uses compare+subtract instead of integer `%`; cold multi‑M `setSeries` (≥1M points) stamps content version in O(1) instead of full FNV (setup hang path for 5M/10M×5). 1M×2500 (~400 pts/bucket) still full-scans. No density period / present lag restored. See `docs/performance.md`.
- **Golden encode-signature / temporal present fidelity (Phase A)** - GPU line decimation no longer amortizes streaming encodes across frames. Whenever prepare `SIG` (algorithm, raw buffer, N, visible window, buckets, contentVersion, ring layout) differs from the last **successful** encode, compute re-runs this frame before draw (`lastEncodedSIG` updates only after encode). Density period table (2/4/32/64), skip streaks, and intentional multi-frame present lag (**period-flash**) are removed for modular FIFO and linear growth. Equal-N version bumps and resource rebuilds still force encode. Multi‑M streaming Avg FPS may drop vs pre-fidelity cadence-inflated numbers; those baselines are **invalid as competitive FPS gates** until a post-present-fidelity harness archive supersedes them. See `docs/performance.md` Golden encode-signature fidelity and `docs/plans/2026-07-27-decimation-temporal-fidelity-goal.md`.
- **GPU-decimation hairline policy uses multi‑M residency N** - When line stroke draws a decimated bucket buffer, dense-hairline entry may fold **raw** point count into policy **only when raw ≥ 1M** (`DENSE_DRAW_POINT_THRESHOLD`) so multi‑M FIFO exits 4× MSAA AA-quad fill while still drawing the small LTTB sample (draw instance count unchanged). Mid-N raw (50k–500k LTTB’d down) keeps full AA quads + configured width when draw N is low.
- **Dense draw LOD auto retune (2a + 2c)** - Raised draw-only dense stride entry from **250k → 1M** points and segment budget from `max(2048, 1× plotWidth)` to **`max(8192, 4× plotWidth)`** under `performance.lod: 'auto'`. Mid-N demos (≤999k, including multi-series playground 250k) keep full N−1 stroke/fill geometry; multi-M still densifies toward the higher pixel budget. Residency/`sampling`/`lod: 'strict'`/`PerformanceLod` API unchanged. Hairline (25k), multi-series hairline budget, and scatter compact floor are separate and unchanged. See `docs/performance.md` Adaptive draw LOD.
